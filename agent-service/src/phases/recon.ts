...
1. Create a new Customer (e.g. name "Recon Test", a unique email).
2. Create a new Product (e.g. name "Recon Test Product", any price).
...
7. Clean up: delete the test Order, then the test Product, then the test
   Customer you created in this step, so recon does not leave test data behind.
...
const USER_MESSAGE = `Start reconnaissance on the QA platform application. Follow the 5 steps in your instructions and return the JSON report.`;
