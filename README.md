...
It combines a deterministic Playwright + Gherkin/BDD regression suite (35 scenarios, generated by
...
- All 35 scenarios are auto-discovered from the `.feature` files; `--scenario` accepts an id, an
  exact title, or a Gherkin tag.

A real (trimmed) diagnosis from this repo's own demo run — a deliberately broken assertion, caught
and correctly classified as a test bug, not an app bug:
...
```json
{
  ...
  "diagnosis": {
    ...
    "confidence": "high",
    "reasoning": "The step definition expects 'SUBMITTED_WRONG' but the API returns 'SUBMITTED' — the Gherkin step says 'the order status should be SUBMITTED', so the test's expected value is the bug, not the app.",
    "proposedPatch": "--- a/tests/steps/orders-status.steps.ts\n+++ b/tests/steps/orders-status.steps.ts\n@@ -58,7 +58,7 @@\n-  expect(body.status).toBe('SUBMITTED_WRONG');\n+  expect(body.status).toBe('SUBMITTED');"
  }
}
```
...
- AI-generated Playwright/BDD suite — 35/35 scenarios passing, hand-hardened after generation
...
- Descriptor editor UI and AI usage/cost dashboard — both live at `docker compose up`
...
That's the whole app stack plus a real 35-scenario Playwright/BDD run. The full walkthrough below
